...
from twitter_core import PublishModule, TimelineMonitor, TwitterClientManager

from .analysis import MonadAnalysisModule
from .research import MonadResearchModule


class MonadTwitterAgent:
    """Twitter Agent - 模拟真实Twitter用户行为（Monad 项目版本）"""

    def __init__(self) -> None:
        self.client_manager = TwitterClientManager()
        self.research = MonadResearchModule()
        self.analysis = MonadAnalysisModule()
    ...
    async def generate_research_tweet(self) -> Optional[str]:
        """生成基于调研的推文"""
        try:
            print("🔍 开始调研Monad相关信息...")
    ...
    async def run_as_twitter_user(self) -> None:
        ...
        try:
            # 任务1：时间线监控（回复评论）
            timeline_task = asyncio.create_task(self.timeline_monitor.run(check_interval=180))

            # 任务2：调研发推
            research_task = asyncio.create_task(self.run_research_loop())
