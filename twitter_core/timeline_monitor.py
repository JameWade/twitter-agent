...
class TimelineMonitor:
    def __init__(self) -> None:
        self.twitter_client: Client | None = None
        self.gemini_client = None
        self.gemini_pool: GeminiKeyPool | None = None
        self.processed_tweets: Set[str] = set()  # 防止重复处理
        self.commented_tweets: Set[str] = set()  # 已评论的推文ID
    ...
    def _load_commented_tweets(self) -> None:
        """加载已评论的推文ID列表"""
        try:
    ...
    def _save_comment_record(self, tweet_data: Dict, comment: str) -> None:
        """保存评论记录到文件"""
        try:
    ...
    def _load_twitter_client(self) -> Client | None:
        """加载Twitter客户端"""
    ...
    async def analyze_tweet_and_generate_comment(self, tweet_data: Dict) -> str | None:
        """分析推文并生成评论"""
...
直接返回评论："""
    ...
    async def monitor_timeline(self, check_interval: int = 300) -> None:
        """监控时间线并自动评论"""
        print("🔍 开始监控时间线...")
        ...
        while True:
            try:
                ...
                if new_tweets:
                    ...
                    tweets_to_comment = (
                        random.sample(new_tweets, comment_count)
                        if len(new_tweets) >= comment_count
                        else new_tweets
                    )
                ...
                print(
                    f"⏰ 下次检查时间: {(datetime.now() + timedelta(seconds=check_interval)).strftime('%H:%M:%S')}"
                )
    ...
    async def run(self, check_interval: int = 300) -> None:
        """启动时间线监控"""
        print("🚀 启动时间线监控系统...")
